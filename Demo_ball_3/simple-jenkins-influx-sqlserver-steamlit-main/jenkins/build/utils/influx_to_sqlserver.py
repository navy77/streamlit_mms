...
class INFLUX_TO_SQLSERVER(PREPARE):
    ...
    def lastone(self) :
        try:
            ...
            for i in range(len(mqtt_topic_value)):
                query = f"select time,topic,lot,d_str1,d_str2,{self.column_names} from mqtt_consumer where topic = '{mqtt_topic_value[i]}' order by time desc limit 1"
    ...
    def df_to_db(self):
        #connect to db
        init_list = ['mc_no','process','lot','d_str1','d_str2']
